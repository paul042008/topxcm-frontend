  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const filteredAlbums = albumsData.filter((a) => a.category === "agbadas");

        const standaloneItems = itemsData.filter(
          (item: any) => !item.album_id && !item.album_name && item.category === "agbadas"
        );
        ...
        <main className="relative z-10 px-4 py-8">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <p className="text-white/40 text-lg font-serif italic">New masterpieces in progress.</p>
            </div>
          ) : (
