  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[200] flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-sm"
        onClick={onClose}
      >
        <motion.div
          ...
          className="relative w-full max-w-full sm:max-w-lg bg-[#111] rounded-2xl overflow-hidden shadow-2xl border border-[#00AEEF]/20 max-h-[95vh] flex flex-col"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-4 py-3 bg-[#1a1a1a] border-b border-white/5">
...
// ─── ALBUM CARD (updated: image clickable, button = Order Now, no badge) ──
...
// ─── SINGLE CARD (updated: image clickable, button = Order Now, no badge) ──
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const standaloneItems = itemsData.filter(
          (item: any) => !item.album_id && !item.album_name && item.category === "suits"
        );
        ...
        <main className="relative z-10 px-4 py-8">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <p className="text-white/40 text-lg font-serif italic">The atelier is preparing new pieces.</p>
            </div>
          ) : (
