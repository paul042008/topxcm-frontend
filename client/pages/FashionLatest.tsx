...
interface Album {
  ...
  cover?: string;
  images: AlbumImage[];
  isSingle?: boolean; // flag to identify quick‑upload singles
}

const API = "https://topxcm-backend-1.onrender.com";

// ─── ITEM MODAL (shared between albums and singles) ──────────────────────
...
function SingleCard({ album, onViewSingle }: { album: Album; onViewSingle: () => void }) {
  // album.images[0] is the single image
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const filteredAlbums = albumsData.filter((a) => a.category === "latest");

        // 4. Filter single items: only those with NO albumId (true standalone) and category latest
        const standaloneItems = itemsData.filter(
          (item: any) => !item.albumId && item.category === "latest"
        );
    ...
    <div className="min-h-screen bg-[#D9EAF0] text-slate-800 overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 w-full z-[100] flex items-center justify-start px-5 py-4 bg-white/50 backdrop-blur-xl border-b border-[#1E3A8A]/5">
        ...
        <div className="flex flex-col items-start ml-4">
          <p className="text-[#00AEEF] text-[10px] tracking-[0.7em] uppercase font-bold">Latest Collection</p>
      ...
      <div className="pt-24 px-5 py-10 border-b border-[#1E3A8A]/5 bg-white/20">
        <h1 className="text-3xl md:text-5xl font-serif italic text-[#1E3A8A] mb-2">Latest</h1>
      ...
      {/* Single item modal (for quick‑upload items) */}
      <AnimatePresence>
        {singleItem && (
