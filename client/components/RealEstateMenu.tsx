      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[10000] bg-white w-screen h-screen overflow-hidden flex flex-col"
          >
            {/* Header Area */}
            <div className="w-full px-8 pt-12 flex justify-between items-center">
              <div className="flex flex-col gap-0.5 opacity-50">
                <span className="text-[#B0D4E8] text-[10px] tracking-[0.7em] uppercase font-bold">TOPXCM</span>
                <span className="text-slate-400 text-[8px] tracking-[0.3em] uppercase">Real Estate</span>
              </div>

              <button 
  onClick={handleClose} 
  className="absolute top-8 right-6 z-[110] ..." // Make sure z-index is high here
>
  <X size={32} />
</button>
            </div>
            ...
            <nav className="flex-grow flex flex-col items-center justify-center gap-10">
              {links.map((link, i) => (
                ...
                >
                  <p className="text-2xl font-black uppercase tracking-[0.3em] text-slate-800 group-hover:text-[#B0D4E8] transition-colors">
                    {link.label}
                  </p>
                  <p className="text-[9px] uppercase tracking-[0.5em] text-slate-400 mt-1">{link.sub}</p>
                </motion.button>
              ))}
              ...
              <div className="mt-8 flex flex-col gap-4 items-center">
                {isRealEstateHome && (
                  <button onClick={() => { handleClose(); navigate("/"); }} className="text-[10px] font-bold uppercase tracking-[0.4em] text-slate-300 hover:text-[#B0D4E8] transition-all">
                    ← Back to Empire
                  </button>
                )}
                {isSubPage && (
                  <button onClick={() => { handleClose(); navigate("/real-estate"); }} className="text-[10px] font-bold uppercase tracking-[0.4em] text-slate-300 hover:text-[#B0D4E8] transition-all">
                    ← Back to Real Estate
                  </button>
                )}
              </div>
